    ...
    if (!redis) {
      const _redis = new Redis(redisURL, {
        ...
        retryStrategy: (times: number) => {
          if (times > maxRetries)
            reject("Maximum retry limit reached for redis.");
          return defaultDelay;
        },
      })
        .on("connect", () => {
        ...
        .on("wait", () => {
          Logger.info(
            "Redis client is waiting for commands to establish connection."
          );
          redis = _redis;
          resolve(redis);
        })
    ...
    if (!DbInstance) {
      ...
      client.on("error", (err) => {
        ...
        if (retryCount > maxRetries)
          reject("Maximum mongo re-connect tries exceeded.");

        const retryTime = expBackOff(retryCount);
        // Retry after some time
        setTimeout(createMongoConnection, retryTime);
        Logger.info(
          `Re-connecting to MongoDB in ${retryTime / MINUTE} minutes.`
        );
      });
    } else {
      resolve();
